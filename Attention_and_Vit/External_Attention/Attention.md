...
注意力机制模块最开始起源自NLP中的 **self-attention** 机制

Attention的核心思想是：**从关注全部到关注重点**。Attention 机制很像人类看图片的逻辑，当看一张图片的时候，我们并没有看清图片的全部内容，而是将注意力集中在了图片的焦点上。
...
**位置像素注意力模块** 更加关注特征图上的像素点与其余像素点的一个相关性，对于输出实现一个全局的响应。

参考：[CV中即插即用的注意力模块 (shimo.im)](https://shimo.im/docs/CJKhpKqYjGxHRdVp/read)



# 注意力机制的应用

参考：[神经网络学习小记录64——Pytorch 图像处理中注意力机制的解析与代码详解_Bubbliiiing的博客-CSDN博客_注意力机制python代码](https://blog.csdn.net/weixin_44791964/article/details/121371986)

注意力机制是一个即插即用的模块，理论上可以放在任何一个特征层后面，可以放在主干网络，也可以放在加强特征提取网络。

## 实例1（YoloV4-tiny）

由于放置在主干会导致网络的预训练权重无法使用，以YoloV4-tiny为例，将注意力机制应用加强特征提取网络上。

如下图所示，我们在主干网络提取出来的两个有效特征层上增加了注意力机制，同时对上采样后的结果增加了注意力机制。

![](E:\Picture_bin\2022_12\0000d4f13f5c874947fcb0194935f1bb44e9.png)
...
参考：[pytorch中加入注意力机制（CBAM），以ResNet为例。到底要不要用ImageNet预训练？如何加预训练参数？_初识-CV的博客-CSDN博客](https://blog.csdn.net/qq_38410428/article/details/103694759)
...
包含 [***Attention***](#attention-series),[***Backbone***](#backbone-series), [***MLP***](#mlp-series), [***Re-parameter***](#re-parameter-series), [**Convolution**](#convolution-series)
...
解读：[深度学习中的 Attention 机制总结与代码实现（2017-2021年） - 知乎 (zhihu.com)](https://zhuanlan.zhihu.com/p/381970151)
